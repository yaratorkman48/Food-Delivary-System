...
public class DeliveryDataBase {
    ...
    /**
     * Wipes all stored data back to an empty state. Called at the START of a
     * load so that loading from disk REPLACES the current data instead of
     * appending to it (which would trip every duplicate-code check). The single
     * system administrator is intentionally preserved, because it is not stored
     * in a text file - it is recreated by this object's constructor.
     */
    public void clearAllData() {
        this.customers.clear();
        this.restAdmins.clear();
        this.restaurants.clear();
        this.riders.clear();
        this.orders.clear();
        this.ordersByCustomer.clear();
        this.restaurantsByCustomer.clear();
    ...
    /**
     * Like findCustomerByCode, but THROWS instead of returning null when the
     * customer is missing (HW3 Part F).
     *
     * The 'throws' clause makes this a CHECKED contract: every caller is forced
     * by the compiler to either catch CustomerNotFoundException or declare it.
     * That is the whole point of a checked exception - the failure cannot be
     * silently ignored the way a forgotten null-check can be.
     *
     * @param code the customer code to look up
     * @return the matching Customer (never null)
     * @throws CustomerNotFoundException if no customer has that code
     */
    public Customer findCustomerByCodeOrThrow(int code) throws CustomerNotFoundException {
        Customer c = findCustomerByCode(code);   // reuse existing logic
        if (c == null) {
            throw new CustomerNotFoundException("No customer exists with code " + code + ".");
        }
        return c;
    }

    /**
     * Like findRestaurantByCode, but THROWS instead of returning null when the
     * restaurant is missing (HW3 Part F).
     *
     * @param code the restaurant code to look up
     * @return the matching Restaurant (never null)
     * @throws RestaurantNotFoundException if no restaurant has that code
     */
    public Restaurant findRestaurantByCodeOrThrow(int code) throws RestaurantNotFoundException {
        Restaurant r = findRestaurantByCode(code);
        if (r == null) {
            throw new RestaurantNotFoundException("No restaurant exists with code " + code + ".");
        }
        return r;
    }

    /**
     * Assigns a rider to an order, THROWING a specific exception for each
     * distinct failure instead of returning a bare boolean (HW3 Part F).
     *
     * This is the checked counterpart of assignRiderToOrder. Where the original
     * collapses every problem into 'return false', this version reports exactly
     * what went wrong via its type, so the menu can show a precise message.
     *
     * @param riderId   the rider's national ID
     * @param orderCode the order to assign
     * @throws DeliveryPersonUnavailableException if the rider is missing or not available
     * @throws IllegalStateException              if the order is missing or already has a rider
     */
    public void assignRiderToOrderChecked(String riderId, int orderCode)
            throws DeliveryPersonUnavailableException {
        ...
        if (!rider.isAvailable()) {
            throw new DeliveryPersonUnavailableException(
                    "Rider '" + riderId + "' is currently unavailable.");
        }
        Order order = findOrderByCode(orderCode);
        if (order == null) {
            throw new IllegalStateException(
                    "Order with code " + orderCode + " was not found.");
        }
        String currentRider = order.getRiderCode();
        if (currentRider != null && !currentRider.isEmpty()) {
            throw new IllegalStateException(
                    "Order " + orderCode + " already has a rider assigned.");
        }
        // All checks passed - perform the same updates as the original method.
    ...
    /**
     * Verifies a customer can afford a given price, THROWING if they cannot
     * (HW3 Part F). A pure check: it changes nothing, it only validates.
     *
     * Kept separate from the actual withdrawal so the menu can validate-then-act
     * inside a try block, and so the shared placeOrder helper does not need its
     * contract changed.
     *
     * @param customer   the paying customer
     * @param finalPrice the price to be charged
     * @throws InsufficientBalanceException if the balance is below finalPrice
     */
    public void verifyBalanceOrThrow(Customer customer, double finalPrice)
            throws InsufficientBalanceException {
        if (customer.getCreditBalance() < finalPrice) {
            throw new InsufficientBalanceException(
                    "Balance (" + customer.getCreditBalance()
                    + ") is below the order's final price (" + finalPrice + ").");
        }
    }

    // ===================================================================
    //  Order transactions (Assignment 4: surfaced for the JavaFX layer)
    // ===================================================================

    /**
     * Creates and commits a new order in ONE place, so every screen that lets
     * someone order (customer screen, restaurant-manager screen) runs the exact
     * same validation and the same four-collection update. Console-free: it
     * THROWS instead of printing, and the JavaFX layer turns the exception into
     * an on-screen alert.
     *
     * @return the committed order
     * @throws InsufficientBalanceException if the customer's balance is too low
     * @throws IllegalArgumentException     if a premium minimum is not met
     */
    public Order placeOrder(Customer customer, Restaurant restaurant,
                            int day, int month, int year, double basicAmount)
            throws InsufficientBalanceException {

        double finalPrice = restaurant.calculateFinalPrice(basicAmount);

        // Premium order below the restaurant's minimum -> calculateFinalPrice returns 0.
        if (finalPrice == 0.0 && restaurant instanceof PremiumRestaurant) {
            PremiumRestaurant pr = (PremiumRestaurant) restaurant;
            throw new IllegalArgumentException("Restaurant '" + restaurant.getName()
                    + "' requires a minimum order of " + pr.getMinimumOrderAmount()
                    + " (you entered " + basicAmount + ").");
        }

        // Affordability check (reuses the existing checked helper).
        verifyBalanceOrThrow(customer, finalPrice);

        // All good: create the order and update all four collections.
        ...
        addOrder(order);
        addOrderToCustomer(customer.getCustomerCode(), order);
        addRestaurantToCustomer(customer.getCustomerCode(), restaurant);
        addPaymentForCustomer(customer.getCustomerCode(), finalPrice);

        return order;
    }

    /**
     * Moves an order to "on the way". Centralised so the rider screen does not
     * reach into Order's status constants directly.
     */
    public void markOrderOnTheWay(Order order) {
        if (order != null) {
    ...
    /**
     * Marks an order delivered, records the date, and frees the rider ONLY if
     * they have no other active (non-delivered) orders left - the same
     * consistency rule used by the HW3 assign-rider flow.
     */
    public void markOrderDelivered(Order order, Rider rider, int day, int month, int year) {
        if (order == null) {
            return;
        }
        order.markAsDelivered(day, month, year);
        if (rider != null && getActiveOrdersForRider(rider.getId()).isEmpty()) {
